...
const PAYOUTS = {
    'Powerball': (matches, special) => {
        if (matches === 5 && special) return 100000000; // Jackpot (estimate)
...
function calculateWinnings() {
    ...
    const updatedHistory = history.map(draw => {
        ...
        // Special case for North 5
        if (gameKey === 'north5') gameKey = 'north5';
        
        const myPicks = CLUB_NUMBERS[gameKey];
        
        if (!myPicks) return draw;
    ...
    console.log('Winnings calculated and history updated.');
}
